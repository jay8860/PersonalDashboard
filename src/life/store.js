...
const normalizeEntries = (entries = []) => [...entries]
  ...
  .map((entry) => ({
    ...
    date: entry?.date || '',
    weightValue: entry?.weightValue || entry?.weightKg || '',
    weightUnit: entry?.weightUnit || 'kg',
    heightValue: entry?.heightValue || entry?.heightCm || '',
    heightUnit: entry?.heightUnit || 'cm',
    waistValue: entry?.waistValue || entry?.waistCm || '',
    aboveNavelValue: entry?.aboveNavelValue || '',
    bicepValue: entry?.bicepValue || '',
    chestValue: entry?.chestValue || entry?.chestCm || '',
    hipValue: entry?.hipValue || entry?.hipCm || '',
    thighValue: entry?.thighValue || '',
    bodyMeasurementUnit: entry?.bodyMeasurementUnit || 'cm',
    note: entry?.note || '',
  }))
